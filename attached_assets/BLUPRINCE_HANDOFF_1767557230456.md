...
## TOSS v1.1 Schema Updates

**CRITICAL**: The attached `toss-v1.1.ts` is the authoritative TypeScript schema.

### v1.1 Changes from v1.0

| Change | Description |
|--------|-------------|
| `items` → `meshes` | Renamed throughout for 3D clarity |
| `tngli_id` | Per-mesh unique path for direct state access |
| `statecharts` | Multi-FSM registry for complex coordination |
| `eventBus` | Cross-chart communication channels |
| `joints` | Spring, hinge, damper physics connections |
| `hardware` | Physical product profiles (MCU, manufacturing) |
| `accessory` | Mountable cart metadata and pricing |
| `cameraSettings` | Per-mesh camera angle storage |
| `stateAnimations` | State→Animation bindings |
| `timeControls.spacetime` | Location + datetime for weather context |
| `soundboard` | Retro sounds with transition bindings |
| `exports.commands` | CLI-like executables |

---

## Weather & Location Context (SpacetimeAnchor)

ThingOS provides real-time weather analysis. Cartridges can be location-aware:

```typescript
interface SpacetimeAnchor {
...
interface TimeControls {
  ...
  spacetime?: SpacetimeAnchor;  // THE KEY: When/where context
  triggers?: {
    on_datetime?: { datetime: string; emit: string }[];
    on_duration?: { after_ms: number; emit: string }[];
  };
}
```
...
```
GET /platform/v1/locations/{id}/weather
→ Returns hourly weather patterns and classifications
```

**Weather-Driven Experiences:**
- Particle effects (rain, snow, fog) based on conditions
- Lighting changes (sunny → overcast transitions)
- Audio ambience (wind, thunder sounds)
- FSM state transitions triggered by weather events
- Temperature-sensitive meshes that react to heat/cold

### Example: Weather-Aware Cartridge

```json
{
  "toss_version": "1.1",
  "meta": { "title": "Weather Demo" },
  "timeControls": {
    "spacetime": {
...
## Multi-Statechart System

v1.1 supports multiple FSMs per cartridge for complex coordination:

```typescript
interface StatechartDefinition {
  id: string;
  role: "ui" | "physics" | "network" | "logic" | "scene" | "custom";
  meshId?: string;           // Linked to specific mesh
  fsm: MeshFSM;
  subscribes?: string[];     // Events to listen for
  publishes?: string[];      // Events this chart can emit
}

interface EventBusConfig {
  channels: string[];
  eventTypes?: Record<string, { payload?: any; source?: string }>;
}
```
...
```json
{
  "statecharts": {
    "player": {
      "role": "logic",
      "meshId": "player_mesh",
      "fsm": { "initial": "idle", "states": { "idle": { "JUMP": "jumping" } } },
      "publishes": ["player:landed"]
    },
    "game": {
      "role": "scene",
      "fsm": { "initial": "playing", "states": { "playing": { "player:died": "gameover" } } },
      "subscribes": ["player:died"]
    }
  },
  "eventBus": { "channels": ["game", "player"] }
}
```
...
Connect meshes with physical constraints:

```typescript
type JointType = "spring" | "hinge" | "fixed" | "slider" | "ball";
...
Retro sounds with state transition bindings:

```typescript
type RetroSoundCategory = 
  | "blip" | "coin" | "jump" | "powerup" | "hit" | "select"
  | "start" | "gameover" | "win" | "error" | "whoosh"
  | "laser" | "explosion" | "magic";

interface SoundboardConfig {
  enabled: boolean;
  masterVolume: number;
  muted: boolean;
  bindings: TransitionSoundBinding[];  // "state:event" → sound
}
```
...
Environmental forces with hitboxes:
...
interface SensitivityDeclarations {
  temperature?: { sensitive: boolean; magnitude: number; onMelt?: string };
  magnetic?: { polarity: "positive" | "negative"; onAttract?: string };
  buoyancy?: { magnitude: number };  // <1 sinks, >1 floats
  aerodynamic?: { dragCoefficient: number };
  electric?: { conductivity: number; onShock?: string };
}
```

---

## Hardware Profile (Physical Products)

For artsy.sale hardware products:

```typescript
interface HardwareProfile {
  productId: string;
  manufacturingData?: {
    stlFiles: string[];      // Asset IDs for 3D printing
...
CLI-like executables exposed by cartridges:

```typescript
interface CommandDefinition {
  id: string;
  tngli_id: string;          // Command name in PATH (e.g., "spawn", "reset")
  description: string;
  args?: CommandArg[];
  permission?: "public" | "authenticated" | "owner" | "admin";
  entrypoint: {
...
Mountable cartridges that extend others:

```typescript
interface AccessoryMetadata {
  isAccessory: boolean;
  provides: { commands?: string[]; panels?: string[]; forces?: ForceType[] };
  compatibleWith?: { cartridgeIds?: string[]; categories?: string[]; all?: boolean };
...
4. **TOSS v1.1 format** - With meshes, statecharts, joints, weather context
5. **4 platform targets** - Web, Desktop, MicroPython, TUI
6. **Commerce ready** - SKU/currency/hardware fields for artsy.sale, unwanted.ad
7. **Weather integration** - SpacetimeAnchor + ThingOS weather API
8. **Multi-FSM coordination** - Statecharts + event bus
9. **Physics joints** - Spring, hinge, damper connections
10. **Audio soundboard** - Retro sounds with transition bindings
